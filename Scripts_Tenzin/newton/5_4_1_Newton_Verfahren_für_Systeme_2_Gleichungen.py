...
#Matrix definieren
f = sy.Matrix([ # ACHTUNG: Für sinus/cosinus/Exponentialfunktion immer sy.sin/sy.cos/sy.exp/sy.ln/sy.abs verwenden!
    1- x1 ** 2 -x2**2,
    ((x1 -2)**2)/2 + (x2-1)**2/4 - 1
])
...
def is_finished(f, x):
    # return is_finished_max_iterations(f, x, 6)      # Abbruchkriterium a): Abbruch nach einer bestimmten Anzahl Iterationen
    # return is_finished_relative_error(f, x, 1e-5)  # Abbruchkriterium b): Abbruch, wenn ‖x(n+1) - x(n)‖₂ ≤ ‖x(n+1)‖₂ * 𝛜
    return is_finished_absolute_error(f, x, 1e-8)  # Abbruchkriterium c): Abbruch, wenn ‖x(n+1) - x(n)‖₂ ≤ 𝛜
    # return is_finished_max_residual(f, x, 1e-5)    # Abbruchkriterium d): Abbruch, wenn ‖f(x(n+1))‖₂ ≤ 𝛜


#####falls plot_implicit gewünscht ist####
plotImplicit = false
#Funktionen von oben wieder einsetzen
...
#grafisch mit plot_implicit
def loran(f1, f2):
    p1 = sy.plot_implicit(sy.Eq(f1, 0), (x1, -2000, 2000), (x2, -2000, 2000))
    p2 = sy.plot_implicit(sy.Eq(f2, 0), (x1, -2000, 2000), (x2, -2000, 2000))
    p1.append(p2[0])
    p1.show()
