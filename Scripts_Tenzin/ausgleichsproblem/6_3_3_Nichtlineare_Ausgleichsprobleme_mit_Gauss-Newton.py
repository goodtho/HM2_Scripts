...
# Messwerte xi definieren
x = np.array([0, 1, 2, 3, 4], dtype=np.float64)

# Messwerte yi definieren
y = np.array([3, 1, 0.5, 0.2, 0.05], dtype=np.float64)

# Funktion definiere
def f(x, a):
    return a[0] * sy.exp(a[1] * x)

# Startvektor für Iteration definieren
lam0 = np.array([3, -1], dtype=np.float64)

# Fehlertoleranz definiere
tol = 1e-5

# Maximale Iterationen  definiere
max_iter = 30
#############################################################################################
...
while increment > tol and k <= max_iter:
    ...
    delta = np.linalg.solve(R, -Q.T @ g_lambda(lam)).flatten() # Achtung: flatten() braucht es, um aus dem Spaltenvektor delta wieder eine "flachen" Vektor zu machen, da g hier nicht mit Spaltenvektoren als Input umgehen kann
